...
#include "shenandoahGenerationType.hpp"
...
void ShenandoahHeap::entry_uncommit(double shrink_before, size_t shrink_until) {
  ...
  ShenandoahConcurrentPhase gc_phase(msg, ShenandoahPhaseTimings::conc_uncommit, GLOBAL_GEN, 0L, true /* log_heap_usage */);
  EventMark em("%s", msg);
...
void ShenandoahHeap::initialize_serviceability() {
  ...
  if (mode()->is_generational()) {
    ...
    _memory_manager->add_pool(_memory_pool);
  }
}

ShenandoahMemoryManager* ShenandoahHeap::memory_manager(ShenandoahGenerationType generation_mode) {
