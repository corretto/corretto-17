...
public:
  ...
  void work(uint worker_id) {
    ...
    // First drain remaining SATB buffers.
    {
      ...
      ShenandoahMarkRefsClosure<GENERATION, NO_DEDUP> mark_cl(q, rp, old_q);
...
void ShenandoahMarkConcurrentRootsTask<GENERATION>::work(uint worker_id) {
  ...
  ShenandoahMarkRefsClosure<GENERATION, NO_DEDUP> cl(q, _rp, old_q);
...
void ShenandoahConcurrentMark::mark_concurrent_roots() {
  ...
  switch (_generation->type()) {
    ...
    case GLOBAL_GEN: {
      ...
      ShenandoahMarkConcurrentRootsTask<GLOBAL_GEN> task(task_queues(), NULL, rp,
                                                         ShenandoahPhaseTimings::conc_mark_roots, workers->active_workers());
      workers->run_task(&task);
      break;
    }
    case GLOBAL_NON_GEN: {
      assert(old_task_queues() == NULL, "Non-generational mark should not have old gen mark queues");
      ShenandoahMarkConcurrentRootsTask<GLOBAL_NON_GEN> task(task_queues(), NULL, rp,
                                                         ShenandoahPhaseTimings::conc_mark_roots, workers->active_workers());
      workers->run_task(&task);
      break;
    }
