...
void ShenandoahEvacuationStats::print_on(outputStream* st) const {
  ...
  st->print_cr("Evacuated " SIZE_FORMAT "%s across " SIZE_FORMAT " objects, "
            ...
            abandoned_count);
  if (_use_age_table) {
    AgeTable* table = const_cast<AgeTable*>(_age_table);
    table->print_age_table(st, ShenandoahHeap::heap()->age_census()->tenuring_threshold());
  }
}
...
void ShenandoahEvacuationTracker::print_evacuations_on(outputStream* st,
  ...
  if (_generational) {
    ...
    young_region_ages.print_age_table(st, tenuring_threshold);
    st->cr();
  }
}
